...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const category = pathArray[
      pathArray.length - 1
    ].toUpperCase() as ContentType;

    // Update the content type if it's a valid category
    if (Object.values(ContentType).includes(category)) {
    ...
    <main className="flex  min-h-screen flex-col ">
      ...
      >
        <div className=" mx-auto flex justify-between">
          <PostNavBottom setFilteredPosts={setFilteredPosts} />
        </div>
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          >
            {filteredPosts?.length > 0 ? (
              <>
                <PoemSeachPost posts={filteredPosts} />
              </>
            ) : (
              <>
                <PoemPost />
